...
export class SourceSniffer {
   ...
   */
  async sniff(url: string): Promise<NormalizedListing | null> {
    try {
  ...
  private async applyStealthBehaviors(page: Page) {
    // Randomized Viewport
    const viewports = [
    ...
    const moveMouseHumanly = async (page: Page) => {
  ...
  private isHighVolumeSource(domain: string): boolean {
    const highVolume = ['linkedin.com', 'indeed.com', 'remoteok.com', 'weworkremotely.com'];
    return highVolume.some(d => domain.includes(d));
  }

  private async parseWithCSS(page: Page, domain: string, url: string): Promise<NormalizedListing | null> {
  ...
  private async parseWithVision(page: Page, url: string): Promise<NormalizedListing | null> {
