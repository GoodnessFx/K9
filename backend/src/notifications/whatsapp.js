...
export async function sendWhatsApp(chatId, message) {
    ...
    try {
        const res = await axios.post(`${BASE}/waInstance${c.id}/sendMessage/${c.token}`, { chatId, message }, { timeout: 12000 });
...
export async function sendSignalToWhatsApp(signal) {
    const c = getCfg();
    if (!c || !c.number)
        return false;
...
function formatSignalForWhatsApp(signal) {
    const categoryLabel = {
