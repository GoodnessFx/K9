...
class SignalEngine {
    isScanning = false;
    opportunityWorker = new OpportunityWorker();
        ...
        try {
            ...
            for (const raw of batch) {
                try {
                    ...
                    const matches = currentSignals.filter(s => s.id !== scored.id &&
                    ...
                    if (scored.category === 'jobs')
                        emitter.emit('newJob', scored);
                    if (scored.category === 'airdrop' || scored.category === 'free')
                        emitter.emit('newAirdrop', scored);
